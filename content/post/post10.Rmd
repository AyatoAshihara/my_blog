...
ここで、$\textbf{K} = \alpha\textbf{X}\textbf{X}^{T} + \beta^{-1}\textbf{I}$で、$\textbf{X}=(\textbf{x}_{1},\textbf{x}_{2},...,\textbf{x}_{N})^{T}$です。対数尤度は
...
$$
\displaystyle\frac{\partial L}{ \partial \textbf{X}} = \alpha \textbf{K}^{-1}\textbf{Y}\textbf{Y}^{T}\textbf{K}^{-1}\textbf{X} - \alpha D\textbf{K}^{-1}\textbf{X}
$$
...
$$
\begin{eqnarray*}
\textbf{K}^{-1}\textbf{X} &=& (\alpha\textbf{X}\textbf{X}^{T} + \beta^{-1}\textbf{I})^{-1}\textbf{X} \\
&=& \textbf{X}(\alpha\textbf{X}\textbf{X}^{T} + \beta^{-1}\textbf{I})^{-1} \\
&=& \textbf{ULV}^{T}(\alpha\textbf{VLU}^{T}\textbf{ULV}^{T} + \beta^{-1}\textbf{I})^{-1} \\
&=& \textbf{ULV}^{T}\textbf{V}(\alpha\textbf{LU}^{T}\textbf{UL} + \beta^{-1}\textbf{I}^{-1})\textbf{V}^{T} \\
&=& \textbf{UL}(\alpha\textbf{L}^{2} + \beta^{-1}\textbf{I})^{-1})\textbf{V}^{T}
\end{eqnarray*}
$$
...
$$
\begin{eqnarray*}
\displaystyle \frac{1}{D}\textbf{Y}\textbf{Y}^{T}\textbf{UL}(\alpha\textbf{L}^{2} + \beta^{-1}\textbf{I})^{-1})\textbf{V}^{T} &=& \textbf{ULV}^{T}\\
\displaystyle \textbf{Y}\textbf{Y}^{T}\textbf{UL} &=& D\textbf{U}(\alpha\textbf{L}^{2} + \beta^{-1}\textbf{I})^{-1})\textbf{L} \\
\end{eqnarray*}
$$

となります。とりあえず、これを見ると$l_{j}$が0でなければ、$\textbf{Y}\textbf{Y}^{T}\textbf{u}_{j} = D(\alpha l_{j}^{2} + \beta^{-1})\textbf{u}_{j}$となり、$\textbf{U}$のそれぞれの列は$\textbf{Y}\textbf{Y}^{T}$の固有ベクトルであり、対応する固有値$\lambda_{j}]は[tex:D(\alpha l_{j}^{2} + \beta^{-1})$ということになります。また、ここから

$$
\displaystyle l_{j} = (\frac{\lambda_{j}}{D\alpha} - \frac{1}{\beta\alpha})^{1/2}
$$
...
となるようです。原典のAppendix.Aを参照しましたが理解できませんでした...（線形代数が弱いんですよね） ここで、$\textbf{U}_{q}$は$N×q$行列
...
へ拡張します。ここで、$\phi(\textbf{x}_{i})$は入力$\textbf{x}_{i}$を非線形変換する関数です。$\phi(・)$は平均$\textbf{0}$、分散共分散行列$\textbf{K}_{\textbf{x}}$のガウス過程と仮定します。PPCAの議論と同様にGPLVMでも
...
dataset <- dplyr::full_join(kigyo.bukka,callrate,by="publication") %>%
  ...
  dplyr::full_join(unemployment_rate,by="publication") %>% 
...
ESTIMATE_GPLVM <- function(Y,P,sigma){
  ...
  # 2. Define log likelyhood function
...
g <- ggplot(gather(as.data.frame(result$output),key = name,value = value),
