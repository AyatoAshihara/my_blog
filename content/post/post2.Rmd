...
1. Constant conditional correlation (CCC) model
...
2. Dynamic Conditional Correlation (DCC) model
元論文はこちら。
...
3. Dynamic Equicorrelation (DECO) model
元論文はこちら
...
d_allocation <- function(ggweight,title){
  ...
  ggplot(ggweight, aes(x=Date, y=weight,fill=ASSET)) +
    ...
    labs(title=title) + facet_wrap(method~.)
}
...
# plot allocation weighting
GG1 <- ggplotly(d_allocation(gmv_weight,"GMV Asset Allocation")) 

setwd("../post/Plotly")
saveWidgetframe(widget = GG1, file = "TEST1.html", selfcontained = TRUE)

```

<iframe seamless src="../Plotly/TEST_widget/index.html" width="100%" height="500"></iframe>
...
# plot allocation weighting
ggplotly(d_allocation(CCC$weight,"CCC")) 
ggplotly(d_allocation(DCC$weight,"DCC")) 
ggplotly(d_allocation(DECO$weight,"DECO"))
ggplotly(d_allocation(benchmark$weight,"benchmark"))
...
riskparity_weight <- rbind(data.frame(CCC$weight,method="CCC",Date=index(CCC$weight)),data.frame(DCC$weight,method="DCC",Date=index(DCC$weight)),data.frame(DECO$weight,method="DECO",Date=index(DECO$weight)))

d_allocation(riskparity_weight, "Asset Allocation")

# plot allocation weighting
GG2 <- ggplotly(d_allocation(riskparity_weight, "Risk Parity Asset Allocation")) 

setwd("../post/Plotly")
saveWidgetframe(widget = GG2, file = "TEST2.html", selfcontained = TRUE)
