...
という風に非線形変換$\phi(\textbf{x}_{i})$をかませて拡張します。$\phi(・)$は平均$\textbf{0}$、分散共分散行列$\textbf{K}_{\textbf{x}}$のガウス過程と仮定します。分散共分散行列$\textbf{K}_{\textbf{x}}$は

$$
$\textbf{K}_{\textbf{x}}$ = \alpha^2\phi(\textbf{x}_{i})\phi(\textbf{x}_{i})^T
$$
